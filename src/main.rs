...
async fn echo(req: Request<hyper::body::Incoming>) -> Result<Response<BoxBody<Bytes, hyper::Error>>, hyper::Error> {
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/") => {
            ...
            let username = body_json.get("username").unwrap();
            let message = body_json.get("message").unwrap();

            println!("username: {}, message: {}", username, message);

            let payload = json!({"timestamp": "20240102"}).to_string();
            Ok(Response::new(full(payload)))
        },
        _ => {
